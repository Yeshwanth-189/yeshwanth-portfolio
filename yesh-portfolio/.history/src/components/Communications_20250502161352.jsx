...
function MusicItem({ song, emojiSet }) {
  ...
  const handleClick = () => {
    const newEmojis = emojiSet.map((emoji, idx) => ({
      id: Date.now() + idx,
      emoji,
      yPos: Math.random() * 80, // Random vertical position
    }));

    setFlyingEmojis(newEmojis);

    // Clear emojis after 10 seconds
    setTimeout(() => {
      setFlyingEmojis([]);
    }, 10000);
  };
    ...
    <li className="music-item" onClick={handleClick}>
      ...
      {flyingEmojis.map((item) => (
        <motion.div
          ...
          initial={{ x: "120vw", opacity: 0, top: `${item.yPos}%` }}
        ...
        >
          <span className="emoji-character">{item.emoji}</span>
        </motion.div>
      ))}
    </li>
  );
                ...
                <div className="music-modal">
                  ...
                  <ul className="music-list">
                    <MusicItem
                      ...
                      emojiSet={[
                        { icon: FaRegLaughSquint, color: "#ff4d4d" },
                        { icon: MdOutlineEmojiEmotions, color: "#66fcf1" },
                        { icon: GiMusicalNotes, color: "#f1c40f" },
                      ]}
                    />
                    <MusicItem
                      song="Night Changes - One Direction"
                      emojiSet={[
                        { icon: GiNightSleep, color: "#8e44ad" },
                        { icon: BsStars, color: "#3498db" },
                        { icon: GiMusicalNotes, color: "#f39c12" },
                      ]}
                    />
                    <MusicItem
                      song="All the Stars - Kendrick Lamar, SZA"
                      emojiSet={[
                        { icon: BsStars, color: "#e74c3c" },
                        { icon: FaMusic, color: "#2ecc71" },
                        { icon: MdOutlineEmojiEmotions, color: "#9b59b6" },
                      ]}
                    />
                    <MusicItem
                      song="Sunflower - Post Malone, Swae Lee"
                      emojiSet={[
                        { icon: GiSunflower, color: "#f39c12" },
                        { icon: BsStars, color: "#1abc9c" },
                        { icon: MdOutlineEmojiEmotions, color: "#e67e22" },
                      ]}
                    />
                    <MusicItem
                      song="Hymn for the Weekend - Coldplay"
                      emojiSet={[
                        { icon: GiMusicalNotes, color: "#2980b9" },
                        { icon: BsStars, color: "#e84393" },
                        { icon: FaMusic, color: "#16a085" },
                      ]}
                    />
                  </ul>
                </div>
