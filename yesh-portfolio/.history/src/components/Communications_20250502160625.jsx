...
function MusicItem({ song, iconSet }) {
  const [showEmojis, setShowEmojis] = useState(false);

  const handleClick = () => {
    setShowEmojis(true);

    // Hide emojis after animation
    setTimeout(() => {
      setShowEmojis(false);
    }, 4000); // 4 seconds then remove
  };
          ...
          {iconSet.map((Icon, idx) => (
            <motion.div
              ...
              transition={{
                duration: 4,
                ease: "easeInOut",
                delay: idx * 0.3, // stagger launch
              }}
            >
              <Icon className="music-emoji" />
            </motion.div>
          ))}
        </div>
      )}
                ...
                <div className="music-modal">
                  ...
                  <ul className="music-list">
                    <MusicItem
                      ...
                      icon={<FaRegLaughSquint className="music-emoji" />}
                    />
                    <MusicItem
                      song="Night Changes - One Direction"
                      icon={<GiNightSleep className="music-emoji" />}
                    />
                    <MusicItem
                      song="All the Stars - Kendrick Lamar, SZA"
                      icon={<BsStars className="music-emoji" />}
                    />
                    <MusicItem
                      song="Sunflower - Post Malone, Swae Lee"
                      icon={<GiSunflower className="music-emoji" />}
                    />
                    <MusicItem
                      song="Hymn for the Weekend - Coldplay"
                      icon={<GiMusicalNotes className="music-emoji" />}
                    />
                  </ul>
                </div>
              )}
