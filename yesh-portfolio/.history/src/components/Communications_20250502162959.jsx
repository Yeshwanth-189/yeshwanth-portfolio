...
function Communications() {
  ...
  const [selectedBackground, setSelectedBackground] = useState(
    "linear-gradient(270deg, #0b0c10, #1f2833)"
  ); // default

  const openModal = (modalName) => {
    setActiveModal(modalName);
    if (modalName !== "music") {
      setSelectedBackground("linear-gradient(270deg, #0b0c10, #1f2833)"); // reset if not music
    }
  };
        ...
        {activeModal && (
          ...
          >
            ...
            >
              ...
              {activeModal === "music" && (
                <div className="music-modal-wrapper">
                  <motion.div
                    className="music-modal-background"
                    animate={{
                  ...
                  <div className="music-modal">
                    ...
                    <ul className="music-list">
                      <MusicItem
                        ...
                        onSongSelect={(bg) => setSelectedBackground(bg)}
                        background="linear-gradient(270deg, #ff4d4d, #ff66cc)"
                      />
                      {/* You can add more MusicItem components here */}
                    </ul>
                  </div>
                </div>
              )}
...
// MusicItem Component outside
function MusicItem({ song, onSongSelect, background }) {
  const handleClick = () => {
    onSongSelect(background);
  };
